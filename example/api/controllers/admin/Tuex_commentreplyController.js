...
module.exports = {
  ...
  index: async function (req, res) {
    if (req.method.toLowerCase() === 'get') {
      var modify_permission = await PermissionService.valid(req.session.admin.id, '/admin/Tuex_commentreply/modify');
      var delete_permission = await PermissionService.valid(req.session.admin.id, '/admin/Tuex_commentreply/delete');
      ...
      try {
        ...
        let data = await Tuex_commentreply.find({
          where: obj,
          limit: limit,
          skip: (page - 1) * limit,
          sort: 'sorted_num desc'
        });
        var count = await Tuex_commentreply.count(obj);
        res.json({
          code: 0,
          msg: '',
          count: count,
  ...
  modify: async function (req, res) {
    ...
    if (req.method.toLowerCase() === 'get') {
      var id = req.param('id');
      let tuex_commentreply = {};
      if (id) {
        tuex_commentreply = await Tuex_commentreply.findOne({id: id});
      }
      return res.view({layout: 'admin/layout', tuex_commentreply: tuex_commentreply});
    } else {
      var id = req.param('id');
      var obj = req.body || {};
      ...
      try {
        let result;
        if (id) {
          result = await Tuex_commentreply.update({id: id}, obj);
        } else {
          obj.create_user = req.session.admin.id;
          result = await Tuex_commentreply.create(obj);
        }
  ...
  delete: async function (req, res) {
    try {
      var id = req.param('id');
      var result = await Tuex_commentreply.update({id: id}, {deleted: 1});
      res.json({
        errCode: 0,
        msg: '删除成功',
        data: result
