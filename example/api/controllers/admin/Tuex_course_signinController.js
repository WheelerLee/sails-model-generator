...
module.exports = {
  ...
  index: async function (req, res) {
    if (req.method.toLowerCase() === 'get') {
      var modify_permission = await PermissionService.valid(req.session.admin.id, '/admin/Tuex_course_signin/modify');
      var delete_permission = await PermissionService.valid(req.session.admin.id, '/admin/Tuex_course_signin/delete');
      ...
      try {
        ...
        var obj = {};
        if (req.param('signin_date') && req.param('signin_date').trim() !== '') {
          obj['signin_date'] = req.param('signin_date').trim();
        }
        ...
        let data = await Tuex_course_signin.find({
          where: obj,
          limit: limit,
          skip: (page - 1) * limit,
          sort: 'sorted_num desc'
        });
        var count = await Tuex_course_signin.count(obj);
        res.json({
          code: 0,
          msg: '',
          count: count,
  ...
  modify: async function (req, res) {
    ...
    if (req.method.toLowerCase() === 'get') {
      var id = req.param('id');
      let tuex_course_signin = {};
      if (id) {
        tuex_course_signin = await Tuex_course_signin.findOne({id: id});
      }
      return res.view({layout: 'admin/layout', tuex_course_signin: tuex_course_signin});
    } else {
      var id = req.param('id');
      var obj = req.body || {};
      ...
      try {
        let result;
        if (id) {
          result = await Tuex_course_signin.update({id: id}, obj);
        } else {
          obj.create_user = req.session.admin.id;
          result = await Tuex_course_signin.create(obj);
        }
  ...
  delete: async function (req, res) {
    try {
      var id = req.param('id');
      var result = await Tuex_course_signin.update({id: id}, {deleted: 1});
      res.json({
        errCode: 0,
        msg: '删除成功',
        data: result
