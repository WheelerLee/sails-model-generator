...
module.exports = {
  ...
  index: async function (req, res) {
    if (req.method.toLowerCase() === 'get') {
      var modify_permission = await PermissionService.valid(req.session.admin.id, '/admin/Banner/modify');
      var delete_permission = await PermissionService.valid(req.session.admin.id, '/admin/Banner/delete');
      ...
      try {
        ...
        if (req.param('link') && req.param('link').trim() !== '') {
          obj['link'] = {contains: req.param('link').trim()};
        }
        if (req.param('state') && req.param('state').trim() !== '') {
          obj['state'] = req.param('state').trim();
        }
        ...
        var count = await Banner.count(obj);
        res.json({
          code: 0,
          msg: '',
          count: count,
  ...
  modify: async function (req, res) {
    ...
    if (req.method.toLowerCase() === 'get') {
      ...
      let banner = {};
      if (id) {
        banner = await Banner.findOne({id: id});
      }
      return res.view({layout: 'admin/layout', banner: banner});
    } else {
      var id = req.param('id');
      var obj = req.body || {};
      ...
      try {
        let result;
        if (id) {
          result = await Banner.update({id: id}, obj);
        } else {
          obj.create_user = req.session.admin.id;
          result = await Banner.create(obj);
        }
  ...
  delete: async function (req, res) {
    try {
      var id = req.param('id');
      var result = await Banner.update({id: id}, {deleted: 1});
      res.json({
        errCode: 0,
        msg: '删除成功',
        data: result
