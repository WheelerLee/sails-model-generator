...
module.exports = {
  ...
  index: async function (req, res) {
    if (req.method.toLowerCase() === 'get') {
      var modify_permission = await PermissionService.valid(req.session.admin.id, '/admin/Course_collection/modify');
      var delete_permission = await PermissionService.valid(req.session.admin.id, '/admin/Course_collection/delete');
      ...
      try {
        ...
        let data = await Course_collection.find({
          where: obj,
          limit: limit,
          skip: (page - 1) * limit,
          sort: 'sorted_num desc'
        });
        var count = await Course_collection.count(obj);
        res.json({
          code: 0,
          msg: '',
          count: count,
  ...
  modify: async function (req, res) {
    ...
    if (req.method.toLowerCase() === 'get') {
      ...
      if (id) {
        course_collection = await Course_collection.findOne({id: id});
      }
      return res.view({layout: 'admin/layout', course_collection: course_collection});
    } else {
      var id = req.param('id');
      var obj = req.body || {};
      ...
      try {
        let result;
        if (id) {
          result = await Course_collection.update({id: id}, obj);
        } else {
          obj.create_user = req.session.admin.id;
          result = await Course_collection.create(obj);
        }
  ...
  delete: async function (req, res) {
    try {
      var id = req.param('id');
      var result = await Course_collection.update({id: id}, {deleted: 1});
      res.json({
        errCode: 0,
        msg: '删除成功',
        data: result
