...
module.exports = {
  ...
  index: async function (req, res) {
    if (req.method.toLowerCase() === 'get') {
      var modify_permission = await PermissionService.valid(req.session.admin.id, '/admin/Tuex_answer/modify');
      var delete_permission = await PermissionService.valid(req.session.admin.id, '/admin/Tuex_answer/delete');
      ...
      try {
        ...
        if (req.param('is_answer') && req.param('is_answer').trim() !== '') {
          obj['is_answer'] = req.param('is_answer').trim();
        }
        ...
        if (req.param('view_fee') && req.param('view_fee').trim() !== '') {
          obj['view_fee'] = req.param('view_fee').trim();
        }
        ...
        let data = await Tuex_answer.find({
          where: obj,
          limit: limit,
          skip: (page - 1) * limit,
          sort: 'sorted_num desc'
        });
        var count = await Tuex_answer.count(obj);
        res.json({
          code: 0,
          msg: '',
          count: count,
  ...
  modify: async function (req, res) {
    ...
    if (req.method.toLowerCase() === 'get') {
      ...
      let tuex_answer = {};
      if (id) {
        tuex_answer = await Tuex_answer.findOne({id: id});
      }
      return res.view({layout: 'admin/layout', tuex_answer: tuex_answer});
    } else {
      var id = req.param('id');
      var obj = req.body || {};
      ...
      try {
        let result;
        if (id) {
          result = await Tuex_answer.update({id: id}, obj);
        } else {
          obj.create_user = req.session.admin.id;
          result = await Tuex_answer.create(obj);
        }
  ...
  delete: async function (req, res) {
    try {
      var id = req.param('id');
      var result = await Tuex_answer.update({id: id}, {deleted: 1});
      res.json({
        errCode: 0,
        msg: '删除成功',
        data: result
