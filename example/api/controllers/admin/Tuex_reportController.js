...
module.exports = {
  ...
  index: async function (req, res) {
    if (req.method.toLowerCase() === 'get') {
      var modify_permission = await PermissionService.valid(req.session.admin.id, '/admin/Tuex_report/modify');
      var delete_permission = await PermissionService.valid(req.session.admin.id, '/admin/Tuex_report/delete');
      ...
      try {
        ...
        var obj = {};
        if (req.param('report_type') && req.param('report_type').trim() !== '') {
          obj['report_type'] = req.param('report_type').trim();
        }
        ...
        let data = await Tuex_report.find({
          where: obj,
          limit: limit,
          skip: (page - 1) * limit,
          sort: 'sorted_num desc'
        });
        var count = await Tuex_report.count(obj);
        res.json({
          code: 0,
          msg: '',
          count: count,
  ...
  modify: async function (req, res) {
    ...
    if (req.method.toLowerCase() === 'get') {
      ...
      let tuex_report = {};
      if (id) {
        tuex_report = await Tuex_report.findOne({id: id});
      }
      return res.view({layout: 'admin/layout', tuex_report: tuex_report});
    } else {
      var id = req.param('id');
      var obj = req.body || {};
      ...
      try {
        let result;
        if (id) {
          result = await Tuex_report.update({id: id}, obj);
        } else {
          obj.create_user = req.session.admin.id;
          result = await Tuex_report.create(obj);
        }
  ...
  delete: async function (req, res) {
    try {
      var id = req.param('id');
      var result = await Tuex_report.update({id: id}, {deleted: 1});
      res.json({
        errCode: 0,
        msg: '删除成功',
        data: result
