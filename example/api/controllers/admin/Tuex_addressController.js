...
module.exports = {
  ...
  index: async function (req, res) {
    if (req.method.toLowerCase() === 'get') {
      var modify_permission = await PermissionService.valid(req.session.admin.id, '/admin/Tuex_address/modify');
      var delete_permission = await PermissionService.valid(req.session.admin.id, '/admin/Tuex_address/delete');
      ...
      try {
        ...
        let data = await Tuex_address.find({
          where: obj,
          limit: limit,
          skip: (page - 1) * limit,
          sort: 'sorted_num desc'
        });
        var count = await Tuex_address.count(obj);
        res.json({
          code: 0,
          msg: '',
          count: count,
  ...
  modify: async function (req, res) {
    ...
    if (req.method.toLowerCase() === 'get') {
      ...
      let tuex_address = {};
      if (id) {
        tuex_address = await Tuex_address.findOne({id: id});
      }
      return res.view({layout: 'admin/layout', tuex_address: tuex_address});
    } else {
      var id = req.param('id');
      var obj = req.body || {};
      ...
      try {
        let result;
        if (id) {
          result = await Tuex_address.update({id: id}, obj);
        } else {
          obj.create_user = req.session.admin.id;
          result = await Tuex_address.create(obj);
        }
  ...
  delete: async function (req, res) {
    try {
      var id = req.param('id');
      var result = await Tuex_address.update({id: id}, {deleted: 1});
      res.json({
        errCode: 0,
        msg: '删除成功',
        data: result
