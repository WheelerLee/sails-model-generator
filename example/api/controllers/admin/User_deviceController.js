...
module.exports = {
  ...
  index: async function (req, res) {
    if (req.method.toLowerCase() === 'get') {
      var modify_permission = await PermissionService.valid(req.session.admin.id, '/admin/User_device/modify');
      var delete_permission = await PermissionService.valid(req.session.admin.id, '/admin/User_device/delete');
      ...
      try {
        ...
        let data = await User_device.find({
          where: obj,
          limit: limit,
          skip: (page - 1) * limit,
          sort: 'sorted_num desc'
        });
        var count = await User_device.count(obj);
        res.json({
          code: 0,
          msg: '',
          count: count,
  ...
  modify: async function (req, res) {
    ...
    if (req.method.toLowerCase() === 'get') {
      ...
      let user_device = {};
      if (id) {
        user_device = await User_device.findOne({id: id});
      }
      return res.view({layout: 'admin/layout', user_device: user_device});
    } else {
      var id = req.param('id');
      var obj = req.body || {};
      ...
      try {
        let result;
        if (id) {
          result = await User_device.update({id: id}, obj);
        } else {
          obj.create_user = req.session.admin.id;
          result = await User_device.create(obj);
        }
  ...
  delete: async function (req, res) {
    try {
      var id = req.param('id');
      var result = await User_device.update({id: id}, {deleted: 1});
      res.json({
        errCode: 0,
        msg: '删除成功',
        data: result
